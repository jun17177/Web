...
이 절에서 필요한 도구를 모두 설치한다. Windows와 macOS 모두 안내한다.
...
**설치 확인** — 터미널(PowerShell 또는 macOS Terminal)을 열고 입력한다:
...
| --- | --- | --- | --- | --- | --- |
| `cd ..` | 상위 폴더로 이동 | `cd ..` | `ls` (macOS) / `dir` (Windows) | 현재 폴더 파일 목록 | `ls` |
| `mkdir 폴더명` | 새 폴더 생성 | `mkdir projects` | `pwd` (macOS) / `cd` (Windows, 인수 없이) | 현재 위치 확인 | `pwd` |
...
1. GitHub([https://github.com)에](https://github.com)에) 로그인한다
